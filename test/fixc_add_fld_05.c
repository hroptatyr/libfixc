...
/* starting out with an empty message, add 2 Quot's */
	...
	/* insert the Side tag */
	fixc_add_tag_at(msg, (fixc_attr_t)FIXML_ATTR_Side, "7", 1, 1);

	if (msg->nflds != 3) {
		fprintf(stderr, "expected 3 fields, got %zu\n", msg->nflds);
		res = 1;
	}
	...
	if (msg->flds[1].tag != FIXML_ATTR_Side) {
		fprintf(stderr, "expected fld[1] to be Side, got %hu\n",
			msg->flds[1].tag);
		res = 1;
	}

	if (msg->flds[2].tag != FIXML_ATTR_Currency) {
		fprintf(stderr, "expected fld[2] to be Ccy, got %hu\n",
			msg->flds[2].tag);
		res = 1;
	}
